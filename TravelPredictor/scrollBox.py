...
class scrollBox(GraphicsObject):
    ...
    def _draw(self, canvas, heha):
        ...
        self.entryPoint = tk.Text(frm,width=self.width,height=self.length, wrap="none")
        self.entryPoint.insert("0.0", "Place Nums")
        self.textVsb = tk.Scrollbar(frm, orient="vertical", command=self.entryPoint.yview)
        self.textHsb = tk.Scrollbar(frm, orient="horizontal", command=self.entryPoint.xview)
        self.entryPoint.configure(yscrollcommand=self.textVsb.set, xscrollcommand=self.textHsb.set)
        self.textHsb.pack(fill="both",side="bottom",padx=(15,0))
        self.textHsb.focus_set()
